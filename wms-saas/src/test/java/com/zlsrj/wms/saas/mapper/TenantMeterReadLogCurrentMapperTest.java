	...
	public void insert() {
		...
		for(int i=0;i<RandomUtil.randomInt(10, 100);i++) {
			...
			TenantMeterReadLogCurrent tenantMeterReadLogCurrent = TenantMeterReadLogCurrent.builder()//
					...
					.meterId(tenantMeter.getId())// 水表ID
					.meterYearTotalWatersBefore(new BigDecimal(RandomUtil.randomInt(0, 1000)))// 结算前水表当年累计水量
					...
					.currentReadPointer(new BigDecimal(RandomUtil.randomInt(0, 1000)))// 本次抄表指针
					.readEmployeeId(RandomUtil.randomLong())// 抄表员ID
					.meterStatusId(tenantMeterStatus.getId())// 表次抄表状况
					.settleWaters(new BigDecimal(RandomUtil.randomInt(0, 1000)))// 应结算水量
					.receivableWaters(new BigDecimal(RandomUtil.randomInt(0, 1000)))// 应收水量
					...
					.processReault(RandomUtil.randomInt(1,2+1))// 处理结果（1：已处理；2：未处理）
